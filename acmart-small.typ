...
#let make_title(title, authors, location, concepts, keywords, body) = {
  ...
  let contents = {
    ...
    parbreak()
    text(size: 10.5pt, font: "Linux Biolinum")[
...
#let acmart(
  ...
  set page(
    height: 10in,
    width: 6.75in,
    margin: (top: 82pt, bottom: 44pt, inside: 46pt, outside: 46pt),
    header-ascent: 16pt,
    numbering: (..args) => {
  ...
  show heading: el => {
    ...
    set text(size: 10.5pt, weight: "bold") if el.level == 1
    set text(size: 10.5pt, weight: "bold") if el.level == 2
  ...
  show figure.caption: set text(font: "Linux Biolinum", hyphenate: false)
